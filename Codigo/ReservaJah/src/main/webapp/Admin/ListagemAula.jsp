        ...
        <c:when test="${listaAula.size()!=0}">
            <c:forEach items="${listaAula}" var="aula">
                <tr>
                    ...
                    <td><a class="modificar" onclick="editarUsuario(<c:out value="${aula.id}"/>);"><img src="../img/editar.png"/></a></td>
                    <td><a class="modificar" onclick="apagarUsuario(<c:out value="${aula.id}"/>);"><img src="../img/apagar.png"/></a></td>
                </tr>
            </c:forEach>
        </c:when>
        <c:otherwise>
